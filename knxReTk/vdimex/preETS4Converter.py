...
class GenericBuilder(object):
    ...
    def getTranslations(self, collection, entityId):
        resultList = []
        translations = list(self.db.text_attribute.find({'entity_id': entityId}, ).sort([('language_id', 1)]))
        for translation in translations:
...
class CatalogBuilder(GenericBuilder):
    ...
    def getCatalogSections(self, obj, key, numbers = [],level = 0, resultDict = {}):
        ...
        for entry in result:
            ...
            translations = self.getTranslations('functional_entity', am.functional_entity_id) #list(self.db.text_attribute.find({'entity_id': am.functional_entity_id}))
            if translations:
                #print "***", translations
                pass
            ...
            for item in self.db.virtual_device.find({"functional_entity_id": entry['functional_entity_id']}, {'virtual_device_id': 1, '_id': 0}).sort([('catalog_entry_id', 1)]):
                translations = self.getTranslations('virtual_device', item['virtual_device_id'])
                ...
                section['items'].append({"Id": vd.catalogItemId, "Name": vd.virtual_device_name, "Number": vd.virtual_device_number,
                    ...
                    "Hardware2ProgramRefId": vd.hardwareProductId,
                })
